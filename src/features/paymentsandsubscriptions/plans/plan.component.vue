...
import { PlanService, SubscriptionService } from '@/features/paymentsandsubscriptions/services/subscription.service';
...
const FALLBACK_PLANS = [
  { id: 'plan_1', planType: 'Básico',      planPrice: '0.00',   paymentFrequency: 'None',
    planLimits: { maxUsers: 10,  maxWarehouses: 5,  maxProducts: 500 } },
  { id: 'plan_2', planType: 'Premium',     planPrice: '5.99',   paymentFrequency: 'Monthly',
    planLimits: { maxUsers: 100, maxWarehouses: 20, maxProducts: 2000 } },
  { id: 'plan_3', planType: 'Enterprise',  planPrice: '42.99',  paymentFrequency: 'Yearly',
...
onMounted(async () => {
  try {
    ...
    plans.value = Array.isArray(raw) && raw.length > 0
...
const continueToPayment = async () => {
  ...
  try {
    ...
    await SubscriptionService.createSubscriptionForAccount(accountId, { selectedPlanId: selectedPlanId.value });
    alert('¡Suscripción activada con éxito!');

    window.location.reload();
  } catch (err) {
    console.error('Error subscribing:', err.response?.data);
    alert('Error al suscribirse: ' + (err.response?.data?.message || 'Error desconocido'));
  } finally {
    isLoading.value = false;
  }
};
      ...
      <div class="plans-container">
        ...
        >
          ...
          <div class="badge-wrapper" v-if="plan.paymentFrequency === 'Yearly'">
            <span class="discount-badge">{{ $t('subscriptions.save_badge') }}</span>
          </div>
          ...
          <div class="card-content">
            ...
            <div class="price-block">
              ...
              <span class="price-freq">
                {{ plan.paymentFrequency === 'Monthly' ? 'subscriptions.per_month' : 'subscriptions.per_year' }}
              </span>
            </div>
            ...
            <ul class="features-list">
              <li v-if="plan.planLimits?.maxUsers">
                ...
                <span>{{ formatLimit(plan.planLimits.maxUsers, 'Usuarios') }}</span>
              </li>
              <li v-if="plan.planLimits?.maxWarehouses">
                <svg viewBox="0 0 24 24" class="check-icon"><path fill="currentColor" d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"/></svg>
                <span>{{ formatLimit(plan.planLimits.maxWarehouses, 'Almacenes') }}</span>
              </li>
              <li v-if="plan.planLimits?.maxProducts">
                <svg viewBox="0 0 24 24" class="check-icon"><path fill="currentColor" d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"/></svg>
                <span>{{ formatLimit(plan.planLimits.maxProducts, 'Productos') }}</span>
              </li>
...
.page-title {
  ...
  letter-spacing: 0px; margin: 0 0 12px;
}
.title-underline {
...
.price-block {
  display: flex; align-items: baseline; justify-content: center; margin-bottom: 8px;
}
.price-val { font-size: 40px; font-weight: 800; color: #ffffff; }
.price-freq { font-size: 14px; font-weight: 600; color: rgba(255,255,255,0.6); margin-left: 6px; }

.plan-desc {
  font-size: 13px; color: rgba(255,255,255,0.6);
  margin: 0 0 24px; min-height: 40px; font-weight: 500;
}

.divider {
  width: 80%; height: 1px; background: rgba(255,255,255,0.1);
  margin-bottom: 24px;
}
...
.check-icon { width: 20px; height: 20px; color: #81c784; flex-shrink: 0; }
.features-list span { font-size: 14px; color: rgba(255,255,255,0.8); font-weight: 500; }
...
.pill-btn {
  width: 100%; padding: 18px 24px; border-radius: 28px;
...
.continue-btn:hover:not(:disabled) {
  background: #ff4f4f; transform: translateY(-3px);
...
.spinner { animation: rotate 2s linear infinite; width: 28px; height: 28px; }
