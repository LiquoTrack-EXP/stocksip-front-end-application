    ...
    <div class="content-wrapper">
      <header class="page-header">
        ...
        <p class="page-subtitle">
          Visualiza y administra tus credenciales de usuario.
        </p>
      ...
      >
        ...
        <div class="card-section">
          ...
          <div class="info-block">
            ...
            <span
              v-else
              class="info-value"
              :class="{ 'empty-value': !user.lastName }"
              >{{ user.lastName || "Sin apellidos" }}</span
            >
          </div>
        </div>
        ...
        <div class="card-footer" :class="{ 'editing-footer': isEditing }">
          ...
          <template v-else>
            <button class="pill-btn edit-btn" @click="toggleEdit">
              Editar Perfil
            </button>
          </template>
        </div>
...
.lang-btn {
  ...
  border: 1px solid rgba(74, 27, 42, 0.2);
  padding: 4px 12px;
  border-radius: 12px;
  cursor: pointer;
  font-weight: 700;
...
.glass-panel.editing-mode {
  box-shadow: 0 24px 48px rgba(229, 62, 62, 0.15);
...
.avatar-header {
  ...
  background: linear-gradient(
    180deg,
    rgba(247, 231, 232, 0.6) 0%,
    rgba(255, 255, 255, 0) 100%
  );
  transition: all 0.3s;
}
.editing-mode .avatar-header {
  background: linear-gradient(
    180deg,
...
.avatar-circle {
  ...
  align-items: center;
  justify-content: center;
  box-shadow: 0 12px 24px rgba(43, 0, 13, 0.12);
  border: 2px solid #2b000d;
  margin-bottom: 20px;
  overflow: hidden;
  color: #d8d0d2;
}
...
.edit-input {
  ...
  padding: 12px 16px;
  border-radius: 12px;
  border: 1px solid rgba(43, 0, 13, 0.15);
...
.edit-btn {
  background: #2b000d;
  color: white;
  box-shadow: 0 8px 24px rgba(43, 0, 13, 0.15);
}
.edit-btn:hover {
  background: #4a1b2a;
  transform: translateY(-2px);
  box-shadow: 0 12px 32px rgba(43, 0, 13, 0.25);
}
