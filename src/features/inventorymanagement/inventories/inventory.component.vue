...
const loadInventory = async () => {
  ...
  try {
    ...
    inventories.value = data.map((item) => ({
      ...item,
      id: item.id || item.inventoryId || item.productId,
      productName: item.productName || item.name || "Producto sin nombre",
      quantity: item.availableStock || item.stock || item.quantity || 0,
      stock: item.availableStock || item.stock || item.quantity || 0,
      unitPrice: item.unitPrice || item.price || 0,
      price: item.unitPrice || item.price || 0,
      moneyCode: item.moneyCode || item.code || "S/.",
      expirationDate: item.expirationDate || null,
      productImageUrl: item.productImageUrl || item.imageUrl || null,
  ...
  <div class="web-layout inventory-layout">
    <div class="content-wrapper">
      <div class="actions-card glass-panel">
        ...
        <div class="actions-grid">
          <button
            ...
            @click="navigateTo(`/inventory_addition/${warehouseId}`)"
          >
          ...
          <button
            ...
            @click="navigateTo(`/inventory_subtrack/${warehouseId}`)"
          >
            <svg viewBox="0 0 24 24" class="action-icon">
          ...
          <button
            ...
            @click="navigateTo(`/inventory_transfer/${warehouseId}`)"
          >
            <svg viewBox="0 0 24 24" class="action-icon">
              <path
                fill="currentColor"
      ...
      <div v-else-if="error" class="state-container glass-panel error-state">
        ...
        <button class="pill-btn retry-btn" @click="loadInventory">
          {{ $t("common.retry") }}
        </button>
      </div>
      ...
      <div v-else-if="inventories.length === 0" class="empty-state glass-panel">
        ...
        <p class="empty-subtitle">
          Toca el botón 'Añadir' en las acciones rápidas para comenzar.
        </p>
      </div>
      ...
      <div v-else class="inventory-grid">
        <div
          ...
          @click="
            navigateTo(`/inventory_detail/${item.id || item.inventoryId}`)
          "
        >
          <div class="card-img-wrapper">
            <img
              v-if="item.productImageUrl || item.imageUrl"
          ...
          <div class="card-info">
            <h4 class="product-name" :title="item.productName || item.name">
              {{ item.productName || item.name }}
            </h4>

            <div class="info-row">
              <span class="info-label">Stock:</span>
              <span class="info-value stock-val">{{
            ...
            <div class="info-row">
              ...
              <span class="info-value price-val"
                >{{ item.moneyCode || "S/." }}
                {{ item.unitPrice || item.price }}</span
              >
            </div>
