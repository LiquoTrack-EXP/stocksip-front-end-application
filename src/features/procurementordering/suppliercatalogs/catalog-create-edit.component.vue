...
const selectWarehouse = async (id) => {
  ...
  try {
    ...
    const [productsRes, warehouseRes] = await Promise.all([
      ProductService.getAccountProducts(accountId),
    ...
    if (Array.isArray(productsRes.data)) {
      ...
      productsArray = productsRes.data.items;
    }

    const accountProductsMap = {};
    productsArray.forEach((p) => {
      const pid = p.id || p.productId;
      accountProductsMap[pid] = p;
    });

    let warehouseProductsArray = [];
    if (Array.isArray(warehouseRes.data)) {
      warehouseProductsArray = warehouseRes.data;
    } else if (warehouseRes.data && Array.isArray(warehouseRes.data.products)) {
      warehouseProductsArray = warehouseRes.data.products;
    } else if (warehouseRes.data && Array.isArray(warehouseRes.data.items)) {
      warehouseProductsArray = warehouseRes.data.items;
    }

    warehouseProducts.value = warehouseProductsArray.map((wp) => {
      const pid = wp.productId || wp.id;
      const accountProduct = accountProductsMap[pid];

      if (accountProduct) {
        return {
          productId: pid,
          name: accountProduct.name,
          unitPrice: accountProduct.unitPrice,
          moneyCode: accountProduct.code || accountProduct.moneyCode || "USD",
          availableStock: wp.availableStock || wp.stock || wp.quantity || 0,
        };
      } else {
        return {
          productId: pid,
          name: wp.name || wp.productName || "Producto sin nombre",
          unitPrice: wp.unitPrice || 0,
          moneyCode: wp.code || wp.moneyCode || "USD",
          availableStock: wp.availableStock || wp.stock || wp.quantity || 0,
        };
      }
    });
...
const confirmProductsSelection = async () => {
  if (!catalogId.value) {
    selectedProductIds.value.forEach((pid) => {
      ...
      const requestedStock = parseInt(productStockInputs.value[pid] || "0", 10);
      if (p && !isProductAlreadyInCatalog(pid)) {
        if (requestedStock > p.availableStock) {
          toast.add({
            severity: "error",
            summary: "Error de Validación",
            detail: `Stock insuficiente para ${p.name}. Disponible: ${p.availableStock}, Solicitado: ${requestedStock}`,
            life: 5000,
          });
          return;
        }
    ...
    for (const pid of selectedProductIds.value) {
      ...
      if (stock > 0) {
        if (stock > p.availableStock) {
          toast.add({
            severity: "error",
            summary: "Error de Validación",
            detail: `Stock insuficiente para ${p.name}. Disponible: ${p.availableStock}, Solicitado: ${stock}`,
            life: 5000,
          });
          continue;
        }
...
const saveCatalog = async () => {
  ...
  try {
    ...
    if (isEditMode.value && catalogId.value) {
      ...
      for (const item of catalogItems.value.filter(
        (i) => i._pendingWarehouseId,
      )) {
        try {
    ...
    if (catalogId.value) {
      try {
        if (isPublished.value)
          await CatalogService.publishCatalog(catalogId.value);
    ...
    toast.add({
      ...
      detail: isEditMode.value
        ? t("catalogs.messages.update_success")
        : t("catalogs.messages.create_success"),
      life: 3000,
    });
    goBack();
  } catch (err) {
...
const deleteCatalog = () => {
  confirmDialog.require({
    message: t("catalogs.messages.delete_confirm"),
    ...
    accept: async () => {
      ...
      try {
        ...
        toast.add({
          severity: "error",
          summary: "Error",
          detail: "No se pudo despublicar el catálogo",
          life: 3000,
        });
      } finally {
        isLoading.value = false;
...
<template>
  <div class="web-layout catalog-create-edit-layout">
      ...
      <div class="form-container">
        ...
        <div class="form-group glass-panel">
          ...
          <input
            ...
            :placeholder="$t('catalogs.fields.description')"
            class="styled-input"
          />
          <div class="divider"></div>
          <input
        ...
        <div class="items-header">
          <h2 class="section-title">
            {{ $t("catalogs.products_in_catalog") }}
          </h2>
          <button
          ...
          <div v-else class="items-list">
            ...
            >
              ...
              <button
                ...
                @click="removeCatalogItem(item.productId)"
              >
        ...
        >
          <span v-if="!isLoading">{{
            isEditMode ? $t("common.save_changes") : $t("common.create_catalog")
          }}</span>
    ...
    <div
      ...
      @click.self="showWarehouseDialog = false"
    >
      <div class="modal-card glass-panel">
        ...
        <div class="modal-actions">
          <button class="text-btn" @click="showWarehouseDialog = false">
            {{ $t("common.cancel") }}
          </button>
        </div>
      </div>
    ...
    <div
      ...
      class="modal-backdrop"
      @click.self="showProductDialog = false"
    >
      <div class="modal-card glass-panel product-modal">
        ...
        <div class="modal-list">
          <div
            ...
            :class="{
              disabled: isProductAlreadyInCatalog(product.productId),
              selected: selectedProductIds.includes(product.productId),
            }"
            @click="
              !isProductAlreadyInCatalog(product.productId) &&
          ...
          >
            <div class="product-item-header">
              ...
              <div class="product-info-mini">
                ...
                <span class="p-price"
                  >{{ product.unitPrice }} {{ product.moneyCode }}</span
                >
                <span class="p-stock"
                  >Available: {{ product.availableStock }}</span
                >
              </div>
              <div class="p-action">
                <span
                  v-if="isProductAlreadyInCatalog(product.productId)"
                  class="added-label"
                  >{{ $t("catalogs.modal.added") }}</span
                >
                <input
                  v-else
                  type="checkbox"
                  :checked="selectedProductIds.includes(product.productId)"
                  @click.stop="toggleProductSelection(product.productId)"
                />
              </div>
            </div>
            ...
            >
              <input
                type="number"
                v-model="productStockInputs[product.productId]"
                :placeholder="$t('catalogs.modal.stock_placeholder')"
                :max="product.availableStock"
                class="styled-input mini-input"
              />
              <span class="stock-hint">Max: {{ product.availableStock }}</span>
            </div>
          </div>
        </div>

        <div class="modal-actions">
          <button class="text-btn" @click="showProductDialog = false">
            {{ $t("common.cancel") }}
          </button>
          <button
            class="pill-btn confirm-btn"
            @click="confirmProductsSelection"
...
.glass-panel {
  ...
  box-shadow: 0 8px 32px rgba(43, 0, 13, 0.06);
  border-radius: 20px;
  overflow: hidden;
}
...
.product-item-card.selected {
  background: rgba(74, 27, 42, 0.05);
  border-color: #4a1b2a;
}
.product-item-card.disabled {
  background: #f0f0f0;
  opacity: 0.7;
  cursor: not-allowed;
}
