<script>
import { CareGuideService, ProductService } from '../services/inventory.service.js';
      ...
      <header class="web-header">
        <div class="flex-title-area">
          <button class="back-btn" @click="goBack" title="Volver al listado">
            ...
            <svg viewBox="0 0 24 24" class="back-icon"><path fill="currentColor" d="M15.41 16.59L10.83 12l4.58-4.59L14 6l-6 6 6 6 1.41-1.41z"/></svg>
          </button>
          <div class="header-text">
            <h1 class="page-title">New Guide</h1>
            <p class="page-subtitle">Configura los parámetros óptimos para preservar la calidad del producto.</p>
          </div>
        </div>
      </header>
        ...
        <div class="image-section">
          ...
          <div class="img-display-box">
             <div class="product-preview">
                <img v-if="form.product === 'Vino Tinto Malbec'" src="https://images.unsplash.com/photo-1584916201218-f4242ceb4809?auto=format&fit=crop&q=80&w=300" alt="Vino" class="mockup-img" />
                <img v-else-if="form.product === 'Ron Cartavio'" src="https://images.unsplash.com/photo-1620067600868-b7eb1c52d43e?auto=format&fit=crop&q=80&w=300" alt="Ron" class="mockup-img" />
                
                <div v-else class="img-placeholder">
                  <svg viewBox="0 0 24 24" class="bottle-icon"><path fill="currentColor" d="M8 21h8v-9l-2-4V5c0-1.1-.9-2-2-2h-0c-1.1 0-2 .9-2 2v3l-2 4v9h0z"/></svg>
                  <span>Seleccione un producto del listado o cargue uno nuevo</span>
                </div>
             </div>
          </div>
        </div>
        ...
        <div class="form-section">
          <form @submit.prevent="saveGuide" class="modern-form">
            ...
            <div class="form-grid">
              ...
              <div class="field-group full-width">
                ...
                <div class="input-wrap select-wrap mockup-shadow">
                  <select v-model="form.productId" required>
                    ...
                    <option v-for="p in products" :key="p.id || p.productId" :value="p.id || p.productId">
              ...
              <div class="field-group full-width">
                ...
                <div class="input-wrap textarea-wrap mockup-shadow">
                  <textarea v-model="form.comments" placeholder="Comments" rows="3"></textarea>
                </div>
              </div>
              ...
              <div class="field-group">
                ...
                <div class="input-wrap icon-input mockup-shadow">
                  <input type="number" step="0.5" v-model="form.minTemp" placeholder="Min. Temperature" required />
              ...
              <div class="field-group">
                ...
                <div class="input-wrap icon-input mockup-shadow">
                  <input type="number" step="0.5" v-model="form.maxTemp" placeholder="Max. Temperature" required />
                  <span class="suffix">°C</span>
                </div>
              </div>
            ...
            <div class="form-actions">
              <button type="button" class="cancel-btn" @click="goBack" :disabled="isLoading">Cancel</button>
              <button type="submit" class="save-btn add-mockup-btn" :disabled="isLoading">
                <span v-if="!isLoading">Add</span>
                <svg v-else class="spinner" viewBox="0 0 50 50" style="width: 24px; height: 24px;"><circle class="path" cx="25" cy="25" r="20" fill="none" stroke-width="5" stroke="white"></circle></svg>
              </button>
            </div>
...
.image-section {
  flex: 0 0 380px; 
  background: linear-gradient(135deg, rgba(235, 227, 228, 0.8) 0%, rgba(244, 236, 236, 0.5) 100%); 
...
.mockup-shadow input, .mockup-shadow select, .mockup-shadow textarea {
  width: 100%;
  padding: 16px 20px;
  border-radius: 12px;
...
.mockup-shadow input:focus, .mockup-shadow select:focus, .mockup-shadow textarea:focus {
  outline: none;
  border-color: rgba(74, 27, 42, 0.3);
  box-shadow: 0 4px 20px rgba(74, 27, 42, 0.12); 
}

.mockup-shadow input::placeholder, .mockup-shadow textarea::placeholder {
